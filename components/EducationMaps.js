...
export function WorldEducationMap({ activeCountry, onCountrySelect, countries, insight }) {
  return <section className="visual-card world-map-card"><div className="visual-card-head"><div><span className="section-tag">World context</span><h3>India and comparable countries</h3></div><span className="tiny-pill">Click a country</span></div><div className="world-map-selection" aria-live="polite"><span>Selected</span><strong>{activeCountry || 'India'}</strong><small>{insight?.metric || 'Choose a country to see the available education context.'}</small></div><div className="world-map-stage"><GeoMap url={WORLD_GEOJSON} ariaLabel="Interactive world map" selected={countries} focus="India" active={activeCountry} onSelect={onCountrySelect} world/></div><p className="micro-copy">Country comparisons are kept separate from state comparisons. They are prompts for learning, not proof that one place can simply copy another.</p></section>;
}
