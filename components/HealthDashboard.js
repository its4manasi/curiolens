...
export default function HealthDashboard() {
  ...
  return <div className="health-live-dashboard">
    <div className="live-index-head"><div><span className="section-tag">{countryName} · latest available</span><h2>Health indicators with their actual publication year.</h2><p>No fixed year is requested. CurioLens uses the newest non-null observation returned by the source for the selected country.</p></div><span className={`live-status ${status}`}>{status === 'ready' ? 'Latest series checked' : status === 'loading' ? 'Checking latest data…' : 'Source temporarily unavailable'}</span></div>
